...
def set_setting(key, value):
    r = sb.table("settings").update({"value": value}).eq("key", key).execute()
    if not r.data:
...
with t5:
    if user == ADMIN_USER:
        ...
        if reg != cur: set_setting("allow_reg", "true" if reg else "false"); st.rerun()
        st.divider()
        ...
        if st.button("发布公告"): set_setting("announcement", new_ann); st.success("公告已发布")
    else:
        st.title("关于")
        ann = sb.table("settings").select("value").eq("key", "announcement").execute().data
        if ann and ann[0]["value"]: st.info(ann[0]["value"])
        st.divider()
        st.write("健身记录 v23 最终版")
        st.divider()
        st.subheader("修改密码")
